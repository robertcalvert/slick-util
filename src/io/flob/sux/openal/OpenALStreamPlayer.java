...
public class OpenALStreamPlayer {
    ...
    private void initStreams() throws IOException {
        ...
        OggInputStream localAudio;

        if (url != null) {
            localAudio = new OggInputStream(url.openStream());
        } else {
            localAudio = new OggInputStream(ResourceLoader.getResourceAsStream(ref));
        }
    ...
    private void removeBuffers() {
        IntBuffer localBuffer = BufferUtils.createIntBuffer(1);
        int queued = AL10.alGetSourcei(source, AL10.AL_BUFFERS_QUEUED);

        while (queued > 0) {
            AL10.alSourceUnqueueBuffers(source, localBuffer);
            queued--;
        }
    }
