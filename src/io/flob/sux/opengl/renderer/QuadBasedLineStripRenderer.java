package io.flob.sux.opengl.renderer;
...
public class QuadBasedLineStripRenderer implements LineStripRenderer {
    ...
    /**
     * True if anti-aliasing is currently enabled
     */
    ...
    /**
     * Render the lines applying anti-aliasing if required
     *
    ...
    public void renderLinesImpl(float[] points, int count, float w) {
        ...
        float step = localWidth <= 12.5f ? 5 : 180 / (float) Math.ceil(localWidth / 2.5);
        ...
        if (lineCaps) {
            ...
            if ((dx != 0) || (dy != 0)) {
                ...
                for (int i = 0; i < 180 + step; i += step) {
                    ...
                    GL.glVertex2f(points[0] + ((float) (Math.cos(ang) * localWidth)),
                            points[1] + ((float) (Math.sin(ang) * localWidth)));
                }
                GL.glEnd();
            }
        }
        ...
        if (lineCaps) {
            ...
            if ((dx != 0) || (dy != 0)) {
                ...
                for (int i = 0; i < 180 + step; i += step) {
                    float ang = (float) Math.toRadians(fang + i);
                    GL.glVertex2f(points[(count * 2) - 2] + ((float) (Math.cos(ang) * localWidth)),
                            points[(count * 2) - 1] + ((float) (Math.sin(ang) * localWidth)));
                }
                GL.glEnd();
            }
        }
